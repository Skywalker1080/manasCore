...
- `journal.py`: `JournalEntry` with `id`, `content`, `created_at`, `updated_at`.
- `emotion.py`: `EmotionRecord` with `entry_id` (FK), `mood`, `emotion`, `sentiment`, `summary`, `tags` (JSON string).

### 1.5 Schemas (`backend/schemas/`)
- Pydantic models for `JournalEntryCreate`, `JournalEntryRead`, and `EmotionRecordRead`.

### 1.6 Journal Router (`backend/routers/journal.py`)
- `POST /entries`: Create raw journal entry (AI processing stubbed for Phase 2).
- `GET /entries`: List entries with pagination.
- `GET /entries/{id}`: Fetch single entry with its emotion record.
...
- [ ] **DB Check:** Run `main.py` and verify `journal.db` is created in `backend/data/`.
- [ ] **Extension Check:** Confirm `sqlite-vec` loads without errors (via a simple `SELECT vec_version()` test).
- [ ] **API Check:** Verify `POST /entries` via Swagger UI (`/docs`).
- [ ] **Full Loop:** Submit a journal entry from the Next.js frontend and see it appear in the "Recent Entries" list.
