...
@router.get("/emotions")
def emotions(range: str = Query("30d", description="Time range"), db: Session = Depends(get_db)):
    """Emotion frequency counts within the requested range."""
    logger.info(f"Fetching emotion analytics for range={range}")
    return get_emotion_frequency(db, range)


@router.get("/tags")
def tags(db: Session = Depends(get_db)):
    """Tag frequency counts across all entries."""
    logger.info("Fetching tag analytics")
    return get_tag_frequency(db)
